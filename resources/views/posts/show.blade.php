<x-app :title="$post->title" :meta-description="$post->body">
    ...
    <div class="flex flex-col max-w-xl px-8 py-4 mx-auto bg-white rounded shadow h-96 dark:bg-slate-800">
        <h4 class="my-4 font-serif text-3xl text-center text-sky-600 dark:text-sky-500">{{$post->body}}</h4>

        <p class="flex-1 leading-normal text-slate-600 dark:text-slate-400">Datos en JSON: {{ ($post) }}</p>
